...
export function Notes({
    ...
    const loadNotes = async () => {
      ...
      try {
        ...
        const {
          ...
          error: fetchError
        } = await supabase.from('decisions').select('notes, user_id').eq('id', decisionId).single();
        if (fetchError) {
  ...
  return <Card className="mt-6">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        {user && !isShared && <Button onClick={saveNotes} variant="outline" size="sm" disabled={isSaving} className="gap-2 rounded-full">
            <Save className="h-4 w-4" />
            {isSaving ? "Sauvegarde..." : "Sauvegarder"}
          </Button>}
      </CardHeader>
      <CardContent>
        {isLoading ? <div className="h-32 w-full bg-muted animate-pulse rounded-md"></div> : <Tabs defaultValue="notes" className="w-full">
            <TabsList className="mb-4 rounded-full gap-1">
              <TabsTrigger value="notes" className="rounded-full">
                {isShared ? "Commentaires" : "Notes personnelles"}
              </TabsTrigger>
              {!isShared && <TabsTrigger value="share" className="rounded-full">Partager</TabsTrigger>}
            </TabsList>
            <TabsContent value="notes">
              <Textarea placeholder={isShared ? "Partagez vos commentaires sur cette décision..." : "Ajoutez vos notes personnelles ici..."} className="min-h-[150px] w-full" value={notes} onChange={e => setNotes(e.target.value)} readOnly={isShared} />
              {isShared && <p className="text-sm text-muted-foreground mt-2">
                  Cette décision a été partagée avec vous. Vous pouvez voir les commentaires mais pas les modifier.
                </p>}
            </TabsContent>
            {!isShared && <TabsContent value="share">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Input value={shareUrl} readOnly placeholder="Sauvegardez d'abord pour générer un lien de partage" />
                    <Button variant="outline" size="sm" onClick={copyShareUrl} disabled={!shareUrl} className="rounded-full">
        ...
        {!user && !isShared && <p className="text-sm text-muted-foreground mt-2">
