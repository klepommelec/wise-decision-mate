...
export function AnalysisResult({
  ...
  const finalScores = useMemo(() => {
    ...
    options.forEach(option => {
      ...
      criteria.forEach(criterion => {
        const evaluation = evaluations.find(e => e.optionId === option.id && e.criterionId === criterion.id);
        if (evaluation) {
  ...
  const criteriaChartData = useMemo(() => {
    ...
    return options.map(option => {
      const evaluation = evaluations.find(e => e.optionId === option.id && e.criterionId === criterionToUse);
      return {
        name: option.title,
  ...
  const saveDecision = async () => {
    ...
    try {
      ...
      const {
        ...
        error: fetchError
      } = await supabase.from('decisions').select('id').eq('title', decisionTitle).eq('user_id', user.id).limit(1);
      if (fetchError) {
      ...
      const {
        error: deleteEvalError
      } = await supabase.from('evaluations').delete().eq('decision_id', decisionId);
      if (deleteEvalError) {
      ...
      } = await supabase.from('criteria').delete().eq('decision_id', decisionId);
      ...
      const {
        data: insertedCriteria,
        error: criteriaError
      } = await supabase.from('criteria').insert(criteriaToInsert).select('id, name');
      if (criteriaError) {
      ...
      const {
        ...
        error: optionsError
      } = await supabase.from('options').insert(optionsToInsert).select('id, title');
      if (optionsError) {
      ...
      if (evaluationsToInsert.length > 0) {
        const {
          error: evalError
        } = await supabase.from('evaluations').insert(evaluationsToInsert);
        if (evalError) {
  ...
  return <div className="w-full max-w-4xl mx-auto animate-fade-in">
      ...
      <BestOption bestOption={bestOption} decisionTitle={decisionTitle} onRegenerateOptions={onRegenerateOptions} />
        ...
        <Card className="border border-gray-200">
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              {user && <Button onClick={saveDecision} variant="outline" className="gap-2" disabled={isSaving}>
          ...
          <CardContent>
            ...
            {/* Criteria chart */}
            <CriteriaChart criteria={criteria} criteriaChartData={criteriaChartData} selectedCriterion={selectedCriterion} setSelectedCriterion={setSelectedCriterion} />
            
            {/* Option details */}
            <OptionDetails finalScores={finalScores} onAddOption={onAddOption} />
          </CardContent>
          <CardFooter className="justify-between pt-4">
            <Button variant="back" onClick={onBack} className="gap-2 rounded-full">
              <ArrowLeft className="h-4 w-4" />
              Retour aux critères
            </Button>
            <DownloadPDF decisionTitle={decisionTitle} contentRef={contentRef} />
          </CardFooter>
        </Card>
      </div>
